...
# 3. ゲーム本体を用意する
#
#    ここでは「動かせる状態にする」ことだけを行う。バージョンを上げるのは
#    /pal update (pal-update.sh) の担当。起動のたびに更新を挟むと、更新失敗が
#    そのまま起動失敗になり、意図しないタイミングでバージョンが上がるため分離した。
#    例外は「キャッシュがセーブに対して古い」場合だけ (下の判定)。それは
#    更新したいからではなく、そのままでは起動できないため選択の余地がない。
# ---------------------------------------------------------------------------
mkdir -p "$PAL_DIR"
