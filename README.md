...
| `/pal update` | ゲーム本体を最新版に更新（2〜4 分・接続者がいない時のみ） |
...
サーバーは**起動しただけでは更新されません**。更新は `/pal update` で明示的に行います。

```text
/pal update
```

更新があるときは起動時に通知が届くので、それを見てから実行すれば十分です。
...
**起動処理から更新を分離している理由**は 2 つあります。更新の失敗がそのまま起動の失敗に
なるのを避けること、そして**意図しないタイミングでバージョンが上がるのを防ぐ**ことです。
新バージョンで一度起動すると旧バージョンに戻せなくなる場合があるため、
更新は「やると決めたときにやる」形にしています。

実行時の動きは次のとおりです。インスタンス自体は落ちないので接続先は変わりません。

1. 接続者がいれば中止（遊んでいる最中に割り込まない）
2. セーブして `saves/archive/pre-update-<旧build>.tar.zst` を作成（30 日保持）
3. ゲームプロセスだけ停止 → 更新 → 再起動

> サーバーが停止しているときは実行できません。停止中はインスタンスごと終了しており、
> 更新を実行する機械が存在しないためです。先に `/pal start` してください。
