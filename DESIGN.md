...
`pal-guardian.service` が 5 秒間隔で 2 つのシグナルを監視する:
...
どちらかが立ったら即座に緊急停止シーケンスへ。フラグファイルで多重起動を防ぐ。
...
| 通知 2 系統（EventBridge） | 予算内（ルール 3） | 「無言で消える」はセーブ喪失の発見遅れに直結するため、セーブ保全の一部とみなす |
...
| 判定 | 根拠 | 通知 |
| --- | --- | --- |
| スポット回収 (直前警告) | IMDS `spot/instance-action` | ⚠ スポット中断の警告 → 自動復帰予告 |
| スポット回収 (予兆による置き換え) | IMDS `events/recommendations/rebalance` | ⚠ サーバー自動移行 → 「誰かが停止したわけではありません」 |
| ユーザー操作 | ASG desired = 0 | 🛑 停止要求 (ユーザー操作) |
| AWS 都合の置き換え | desired ≥ 1 のままの終了 | 🔁 AWS による置き換え → 自動復帰予告 |
