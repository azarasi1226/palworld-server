...
パルワールド本体は約 5GB（展開後）。毎回 steamcmd で取得すると 5〜8 分かかる。
そこでインストール済みディレクトリを `gamecache/palserver.tar.zst` として S3 に置き、
起動時はそれを展開するだけにする。**起動処理では更新しない**（`pal-update.sh` の担当）。
キャッシュの作り直しは起動後の `pal-post-boot.sh` が担い、
`appmanifest_2394010.acf` の buildid が `gamecache/buildid.txt` と違うときだけ行う。

**展開前にキャッシュの世代を確認する。** セーブが要求する世代
（`saves/latest.tar.zst` のメタデータ `game-buildid`）よりキャッシュが古い場合、
展開しても `Save data version mismatch` でクラッシュループになる。
その場合はキャッシュを使わず、「キャッシュが無いとき」と同じクリーンインストール
経路へ合流させる（最新版が入るのでセーブと必ず整合する）。判定を展開前に置くことで、
セーブを退避・復元しながら入れ替える処理が不要になる。

**実測 45〜65 秒**（初回のみキャッシュ生成のため 6〜8 分）。内訳と律速は次のとおり。

| 区間 | 時間 | 律速 |
| --- | --- | --- |
| EC2 起動・cloud-init | 21〜29s | AWS 側。手を入れられない |
| aws-cli 導入・スクリプト取得 | 11〜12s | |
| OS 準備 | 2s | swap 作成のみ（必要なコマンドは AMI に揃っている） |
| キャッシュ展開 | 25〜36s | **S3 からの単一ストリーム転送**（個体差あり） |
| セーブ復元 | 4s | 予兆による移行時のみロック待ちが加算される |
| サーバー起動・RCON 応答待ち | 16〜21s | ゲーム本体の起動時間 |

展開はディスクではなく転送で律速している（4.9GB がページキャッシュに収まるため
書き込みを待たない）。gp3 のスループットを上げても効果はない。並列度を上げても
標準出力へ流す方式では並列化できないため変わらない（10 / 30 / 60 で実測差なし）。
...
| セーブ肥大化 (長期運用で Level.sav が 1GB 超) | 緊急セーブが 2 分に収まらなくなる | 現状 1.2MB / 緊急セーブは実測 12 秒 (猶予 2 分に対し 10 倍の余裕) なので閾値監視は置かない。`status.json` にサイズを載せてあり `/pal status` で見える |
