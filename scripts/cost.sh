...
#   [B] 今この瞬間 (EC2 API)  ... 遅延ゼロ。稼働中インスタンスの時間単価と当セッション分
#
...
echo "==================================================================="
echo
echo "[A] 今月の実績  ($MONTH_START 〜)   ※ Cost Explorer は反映が半日〜1日遅れます"
echo "-------------------------------------------------------------------"
...
print(f"    {'-' * 30} {'-' * 9}")
print(f"    {pad('合計', 30)} {yen(rate):>9}/時")
print()
print(f"     稼働時間: {int(up_h)}時間{int(up_h % 1 * 60)}分")
'
fi
